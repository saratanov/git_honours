...
#TODO: new bond feature / atom feature functions, directed MPNN, double MP + NN
...
def load_dataset():
    """Loads the dataset using a random splitter"""
...
def readout(h, h2):
    """Readout function for two vectors - concatenates vectors,  applies selu activation, adds together all elements, returns tanh"""
...
def message_pass(g, h, k):
    ...
    Params
    ------
    g: ordered dictionary of atom features
    h: ordered dictionary of bond features
    k: index?
    """
    for v in g.keys():
        neighbors = g[v]
...
def construct_multigraph(smile):
    ...
    for i in range(0, molecule.GetNumAtoms()):
        ...
        for j in range(0, molecule.GetNumAtoms()):
            ...
            if e_ij != None:
#                e_ij =  map(lambda x: 1 if x == True else 0, dc.feat.graph_features.bond_features(e_ij))
...
#        r2_val_new = pearsonr(y_val, y_hats_val)[0]**2
        ...
        print('epoch [%i/%i] train_loss [%f] val_loss [%f] r2_val_old [%.4f]' % (num_epoch, 100, train_loss_, val_loss_, r2_val_old))
        num_epoch += 1