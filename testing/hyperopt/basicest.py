...
from model_helpers import delfos_data
...
def data_maker(solute, solvent, pka, ids=None):
    ...
    #sentences
    sentence_pairs = delfos_data(solute,solvent)
...
def train(model, ids, data, scaler, datasets):
    ...
    if model.model_type == 'torch':
        ...
        early_stopping = EarlyStopping(name,patience=10)
...
class EarlyStopping:
    ...
    def store(self, loss, net):
        if loss < self.best_loss:
            self.best_loss = loss
            self.steps = 0
            torch.save(net.state_dict(), 'checkpoints/'+self.name+'.pt')
        else:
            self.steps += 1
